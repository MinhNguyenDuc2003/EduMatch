    ...
    <div className="bg-gray-50 rounded-lg p-6 border border-gray-200 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-4">
          {/* Provider Icon with Verified Badge */}
          <Card
          ...
          >
            {provider.logoUrl ? (
              <Image
                ...
                alt={provider.organizationName || 'Provider logo'}
                fill
          ...
          <div className="flex-1 cursor-pointer" onClick={() => onViewProvider?.(provider.id)}>
            <h3 className="font-semibold text-gray-900 hover:text-blue-600 transition-colors">
              {provider.organizationName || 'Organization Name'}
            </h3>
          </div>
          {/* Follow Button */}
      ...
      {news.scholarship && (
        <div className="pt-4 border-t border-gray-200">
          <h4 className="font-semibold text-gray-900 text-sm mb-3">Related Scholarship</h4>
          <CardSmalPic
            scholarship={news.scholarship}
            onViewDetails={() => {
              if (onViewScholarship && news.scholarship?.slug) {
                onViewScholarship(news.scholarship.slug);
              }
            }}
